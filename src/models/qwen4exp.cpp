...
void llama_model_qwen4exp::load_arch_tensors(llama_model_loader & ml) {
    ...
    if (hparams.ple_n_heads > 0) {
        ...
        // Hoisted out of the `if` because the on-disk path below needs the weight's file index
        // and offset to pread from, which upstream's scoped pointer would not reach.
        ...
        // --ngram-on-disk needs a real tensor to read rows out of; a synthesised model has none,
        // so fall back to the lazy path rather than failing
    ...
    for (int il = n_layer; il < n_layer + (int) hparams.n_layer_nextn; ++il) {
        ...
        // n_ff_exp became a per-layer array upstream (c61b98b87); this call site still read the scalar
        const int64_t n_ff_exp   = hparams.n_ff_exp() ? hparams.n_ff_exp() : n_ff / n_expert_used;
        const int64_t n_ff_shexp = hparams.n_ff_shexp ? hparams.n_ff_shexp : n_ff;
        const int64_t idx_dim    = hparams.indexer_head_size;
